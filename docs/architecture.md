...
- **CI gate before review.** `submit_for_review` validates the clean committed head, pushes it,
  ...
  implementer runs only the test files that cover its change before submitting, not lint, typecheck,
  the suite or builds: CI is the check, and a red CI comes back to the same session.
