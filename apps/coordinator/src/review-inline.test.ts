...
test.each(["reviewer-inline", "reviewer-dirty"])(
  ...
  async (name) => {
    ...
    try {
      ...
      expect(state.review?.reviewerCommits).toEqual([head]);
      expect(state.findings).toMatchObject([
        { status: "fixed", resolution: { commitSha: head } },
      ]);
      expect(
      ...
      expect(state.artifactContents.handoff).toMatchObject({
        reviewerSubmission: {
          input: { reviewedSha: head, reviewerCommits: [head] },
        },
      });
