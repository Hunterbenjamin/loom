...
test("a headless run that crashes is retried on the same session", async () => {
...
test("a message the provider never confirms is resent once and then raises attention", async () => {
  const h = await harness();
  const taskId = start(h);
...
test("a duplicate event changes nothing and a rate limit blocks until it resets", async () => {
  const h = await harness();
  const taskId = start(h);
...
test("an interactive run that disappears ends vanished and asks for a human", async () => {
  const h = await harness();
  const taskId = start(h, {
    planner: "claude",
    implementer: "claude",
  ...
  const driver = new ScenarioDriver(h, await scenarios("vanished-interactive"));
  await driver.run({
...
test("a blocking question blocks the task until the human answers it", async () => {
  const h = await harness();
  const taskId = start(h);
